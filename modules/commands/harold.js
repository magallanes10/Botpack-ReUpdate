...
Just type <harold can you download this facebook video (fb video url)> or <midoriya download this facebook video (fb video url)> as long as there is the word ‘facebook video’ or ‘download’ and the fb video url, it can automatically detect it.


•HAROLD CAN SEARCH IMAGES.

Just type <harold can you search an image of (your search)> or <midoriya image of (your search)> as long as there is the word ‘image’ or ‘image of’ and your search, it can automatically detect it.


•HAROLD CAN SEARCH VIDEO FROM YOUTUBE.

Just type <harold can you send a video of (your search)> or <midoriya video of (your search)> and more... as long as there's the word ‘video of’ and you search, it will automatically send a video
...
Just type <harold can you give me the lyrics of (song name)> or <midoriya give me a lyrics of (song name)> as long as there's a word ‘lyrics of’ and the song name it will be automatically detected.
...
Just type <harold send the tiktok video of (your search)> or <midoriya send the tiktok video of (your search)> or <midoriya send a tiktok video of (your search)>
...
var ok = rest.videos[random]
  const stream = ytdl("https://www.youtube.com/watch?v="+ok.id, { filter: 'audioonly' });
    api.sendMessage("Yeah Sure just please wait...", event.threadID, event.messageID);
...
// console.log(textAfterTikTok);
  try {
    const rest = await axios.get(`https://free-api.ainz-sama101.repl.co/tiktok/tiktokdl?link=${encodeURIComponent(url)}`)
    var vid = rest.data.wmplay
    var title = rest.data.title
      const re = (await axios.get(encodeURI(vid), {responseType: "arraybuffer"}));
      fs.writeFileSync(tikvid);
